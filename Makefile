...
DOCKER_EXEC = $(DOCKER_COMP)  exec app_php
...
start: down exist-image network check_parameter ## Start the stack-php make start project=<folder>
	@set -e; \
 	if [ ! -d "my-project/$(project)" ]; then \
		...
		URL_LOCAL_WEBSITE=$(URL_LOCAL_WEBSITE) ENABLE_LOCAL_SERVER=$(ENABLE_LOCAL_SERVER) PROJECT=$(PROJECT) DOCKER_ENV=$(DOCKER_ENV) PHP_VERSION=$(PHP_VERSION) $(DOCKER_COMP_IP) -f "my-project/$(project)/docker-compose.yml" --profile runner up --detach; \
	fi
...
install_folder_project:
	@./bin/createProject.sh --url_website $(url_website) --project $(project) --phpversion $(phpversion) --env $(env) --enable_quality $(enable_quality)  --enable_local_server $(enable_local_webserser)
